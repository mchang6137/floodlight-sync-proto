...
public class DistMemStorageSource extends NoSqlStorageSource {
    private DistSyncService sync;
    ...
    public void init(FloodlightModuleContext context)
		...
		super.init(context);
		IFloodlightProviderService controller = context.getServiceImpl(IFloodlightProviderService.class);
		
    	Map<String, String> config = context.getConfigParams(this);
    	final DistMemStorageSource _this = this;
    	
    	sync = new DistSyncService(config.get("sync"), new TableSource() {
			@Override
			public MemoryTable getTable(String name) {
				return _this.getTable(name, false);
			}

			@Override
			public MemoryTable createTable(String name) {
				return _this.getTable(name, true);
			}    		
    	}, controller);
    	sync.start();
    }
    ...
    protected void insertRows(String tableName, List<Map<String,Object>> insertRowList) {
        ...
        synchronized (table) {
            for (Map<String,Object> row : insertRowList) {
                ...
                sync.send(Update.insertRow(tableName, primaryKey, row));
            }
        }
    }
    ...
    protected void updateRows(String tableName, Set<Object> rowKeys, Map<String,Object> updateRowList) {
        ...
        synchronized (table) {
            for (Object rowKey : rowKeys) {
                ...
                sync.send(Update.updateRow(tableName, rowKey, updateRowList));
            }
        }
    }
    ...
    protected void deleteRowsImpl(String tableName, Set<Object> rowKeys) {
        ...
        synchronized (table) {
            for (Object rowKey : rowKeys) {
                ...
                sync.send(Update.deleteRow(tableName, rowKey));
            }
        }
    }
